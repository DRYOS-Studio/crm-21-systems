...
Faça UMA pergunta de descoberta sobre o que falta, e devolva "etapa": "descobrir".`;
  ...
  const precisaEscalar = r.escalar === true || confirmacoes >= 2 || pediuHumano((() => { const i = historyMessages.map((m) => m.role).lastIndexOf("assistant"); return historyMessages.slice(i + 1).filter((m) => m.role === "user").map((m) => m.content).join("\n"); })());
...
function __sair(t: string) { return [/^\s*(pare|para|parar)\s*[.!]*\s*$/i, /\bpare de\b/i, /\bpara de me\b/i, /n[aã]o quero mais receber/i, /n[aã]o tenho interesse/i,
    ...
    /perde(u|r) meu tempo/i, /n[aã]o perturbe/i].some((r) => r.test(t)); }
