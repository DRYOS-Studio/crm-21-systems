...
async function main() {
  ...
  try {
    const r = await get(`${url}/rest/v1/`, { headers: { apikey: key } });
    // Chaves novas `sb_publishable_*` recusam GET em /rest/v1/ ("Secret API key
    // required") mas autenticam nas tabelas. Confirma no schema, não na raiz.
    if (r.status === 401 && key.startsWith("sb_publishable_")) {
      const probe = await get(`${url}/rest/v1/profiles?select=*&limit=0`, { headers: { apikey: key } });
      if (probe.ok || probe.status === 404) ok("API REST respondendo (publishable key)");
      else fail("A publishable key foi recusada nas tabelas. Confira se a chave é deste projeto.");
    } else if (r.status === 401) fail("A anon key foi recusada (401). Confira se a chave é deste projeto.");
    else if (!r.ok) fail(`API respondeu ${r.status}. O projeto existe e está ativo?`);
    else ok("API REST respondendo");
  } catch (e) {
